...
안녕하세요. C++를 이용한 성능 최적화와 실시간 시스템 개발에 관심을 가지고 있는 지원자 장경찬입니다.

처음업무로 C/C++를 사용하면서 언어를 접했지만, 이후 개인적으로 암호화폐 거래소 API 프로젝트를 진행하면서 C++에 더욱 흥미를 갖게 되었습니다.

Boost.Asio를 이용해 REST API와 WebSocket을 직접 구현하고, 비동기 통신 구조와 메모리 관리, 객체 설계를 고민하면서 자연스럽게 성능과 안정성을 중요하게 생각하게 되었습니다.

또한 프로젝트를 진행하면서 std::move, 스마트 포인터, 템플릿, 비동기 프로그래밍뿐 아니라 최근에는 소켓에서 수신되는 데이터를 하나의 스레드에서 모두 처리하는 구조의 한계를 경험하고 수신 스레드와 처리 스레드를 분리해야 하는 이유를 고민하게 되었고, 

그 과정에서 Lock-Free Queue, 메모리 오더링, CPU의 메모리 재배치와 같은 주제까지 공부하며 C++를 더 깊이 이해하려고 노력하고 있습니다.
